...
digiChamber = events.digiChamber[BX_mask]
digiEta = events.digiEta[BX_mask]
...
#Remove nones
clusterStrips = ak.fill_none(clusterStrips[~ak.is_none(clusterStrips, axis=2)], 0.0)
clusterBXs = ak.fill_none(clusterBXs[~ak.is_none(clusterBXs, axis=2)], 0.0)
clusterChambers = ak.fill_none(clusterChambers[~ak.is_none(clusterChambers, axis=2)], 0.0)
clusterEtas = ak.fill_none(clusterEtas[~ak.is_none(clusterEtas, axis=2)], 0.0)



clusterStrip_min = ak.fill_none(ak.min(clusterStrips, axis=2), 0.0)
clusterStrip_avg = ak.fill_none(ak.mean(clusterStrips, axis=2), 0.0)
clusterStrip_size = ak.fill_none(ak.num(clusterStrips, axis=2), 0.0)

clusterBX_min = ak.fill_none(ak.min(clusterBXs, axis=2), 0.0)
clusterBX_max = ak.fill_none(ak.max(clusterBXs, axis=2), 0.0)
...
clusterBX_second = ak.fill_none(clusterBX_second, 0.0)
#clusterBX_second = ak.pad_none(ak.sort(clusterBXs, axis=2), 2, axis=2)[:,:,1]
clusterBX_avg = ak.fill_none(ak.mean(clusterBXs, axis=2), 0.0)

clusterChamber = ak.fill_none(ak.mean(clusterChambers, axis=2), 0.0)
clusterEta = ak.fill_none(ak.mean(clusterEtas, axis=2), 0.0)
